...
func selectDB(cli *Client, args [][]byte) (interface{}, error) {
	...
	// 若当前的db为空则创建一个
	if db == nil {
		path := filepath.Join(cli.svr.opts.dbpath, fmt.Sprintf(dbName, 0))
		opts := rose.DefaultOptions(path)
		newdb, err := rose.Open(opts)
		if err != nil {
			return nil, err
		}
		db = newdb
...
func appendStr(cli *Client, args [][]byte) (interface{}, error) {
	...
	return redcon.SimpleString(cli.db.StrLen(key)), nil
}
...
func rPush(cli *Client, args [][]byte) (interface{}, error) {
	...
	return redcon.SimpleString(cli.db.LLen(key)), nil
}
...
// key 之后再接一个int 参数，可表示pop出多个
func popInternal(db *rose.RoseDB, args [][]byte, isLeft bool) (interface{}, error) {
	if len(args) < 1 {
		return nil, newWrongNumOfArgsError("args")
	}
	key := args[0]
	var count = 1
	if len(args) == 2 {
...
// |--------------------------- Hash commands ----------------------------|
// het key field value , field value .....
...
func hStrLen(cli *Client, args [][]byte) (interface{}, error) {
	if len(args) != 2 {
		return nil, newWrongNumOfArgsError("hsetlen")
	}
...
// input key count withValues(bool)
// return field or field and value
...
func sMembers(cli *Client, args [][]byte) (interface{}, error) {
	if len(args) != 1 {
		return nil, newWrongNumOfArgsError("smember")
	}
...
func sCard(cli *Client, args [][]byte) (interface{}, error) {
	...
	return redcon.SimpleInt(cli.db.SCrad(args[0])), nil
}
...
// |------------------------- Sorted Set commands ------------------------|
// input key score member , score member ,score member
func zAdd(cli *Client, args [][]byte) (interface{}, error) {
	...
	key := args[0]
	for i := 1; i < len(args); i += 2 {
		score, err := util.StrToFloat64(string(args[1]))
		if err != nil {
			return nil, errValueIsInvalid
		}
